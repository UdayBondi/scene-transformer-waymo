...
class Encoder(nn.Module):
    ...
    head_num=4, max_dynamic_rg=16, max_static_rg=1400, k=4): 
        ...
        self.pos_enc = PositionalEncoding(feature_dim, max_len=time_steps)
    ...
    def forward(self, state_feat, agent_batch_mask, padding_mask, hidden_mask, 
        ...
        state_feat[hidden_mask] = -1  # The paper sets this to 0. Did not find change in result
        ...
        A_ = self.pos_enc(A_.permute(1, 0, 2)).permute(1, 0, 2)
        B_ = self.pos_enc(B_.permute(1, 0, 2)).permute(1, 0, 2)
        C_ = self.pos_enc(C_.permute(1, 0, 2)).permute(1, 0, 2)
