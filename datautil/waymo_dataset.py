...
def waymo_collate_fn(batch, time_steps=10, current_step=3, sampling_time=0.5, GD=16, GS=1400, is_local=True, halfwidth=50, only_veh=True, hidden='BP'): # GS = max number of static roadgraph element (1400), GD = max number of dynamic roadgraph (16)
    ...
    for data in batch:
        ...
        if hidden == 'MP':
            ...
            raise KeyError('hidden should be MP or CMP or GCMP')
        ...
        if is_local:
            # 1. Translate and divide x,y of agents, roadgraph elements as per the AV
            # 2. Only consider data that is in the vicinity of the AV. 
            # 3. This ensures x,y of agents, roadgraph elements are in the range (-1,1)
